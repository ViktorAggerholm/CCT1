...
// main.ts
var parseRemarkPos = (p) => {
  return { line: p.line - 1, ch: p.column };
};
var HeaderNavPlugin = class extends import_obsidian.Plugin {
  async onload() {
    this.addCommand({
      id: "back-to-heading",
  ...
  backToHeading(editor) {
    const tree = this.getTree(editor);
    const point3 = this.findHeadingBefore(editor, tree, editor.getCursor());
    if (point3) {
      editor.setCursor(point3);
    }
  }
  forwardHeading(editor) {
    const depth = this.depth(editor, editor.getCursor().line);
    const tree = this.getTree(editor);
    const point3 = this.findHeadingAfter(editor, tree, editor.getCursor(), depth);
    if (point3) {
      editor.setCursor(point3);
    }
  }
  backwardHeading(editor) {
    const depth = this.depth(editor, editor.getCursor().line);
    const tree = this.getTree(editor);
    const point3 = this.findHeadingBefore(editor, tree, editor.getCursor(), depth);
    if (point3) {
      editor.setCursor(point3);
    }
  }
  upHeading(editor) {
    const depth = this.depth(editor, editor.getCursor().line);
    const tree = this.getTree(editor);
    const point3 = this.findHeadingBefore(editor, tree, editor.getCursor(), depth - 1);
    if (point3) {
      editor.setCursor(point3);
    }
  }
  findHeadingBefore(editor, tree, position2, depth) {
    let before = void 0;
    const headings = this.flatHeadings(tree);
    headings.forEach((p) => {
      if (p.line < position2.line) {
        const p_depth = this.depth(editor, p.line);
        if (depth === void 0 || p_depth === depth) {
          before = p;
        }
      }
    });
  ...
  findHeadingAfter(editor, tree, position2, depth) {
    let after = void 0;
    const headings = this.flatHeadings(tree);
    headings.forEach((p) => {
      if (p.line > position2.line) {
  ...
  firstChild(editor) {
    const depth = this.depth(editor, editor.getCursor().line);
    const tree = this.getTree(editor);
    const point3 = this.findHeadingAfter(editor, tree, editor.getCursor());
    if (depth && point3 && this.depth(editor, point3.line) > depth) {
      editor.setCursor(point3);
    }
  }
  pointMax(editor) {
  ...
  depth(editor, line) {
    const content3 = editor.getLine(line);
    let i = 0;
