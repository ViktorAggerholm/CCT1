...
| Category          | Mnemonic   | Description                                 | Sets Condition Codes? |
| ----------------- | ---------- | ------------------------------------------- | --------------------- |
| **Operate**       | `ADD`      | Adds source operands and stores result      | Yes                   |
|                   | `AND`      | Performs bitwise AND on source operands     | Yes                   |
|                   | `NOT`      | Performs bitwise NOT on source operand      | Yes                   |
| **Data Movement** | `LD`       | Loads data from memory (PC-relative)        | Yes                   |
|                   | `LDI`      | Loads data indirectly from memory           | Yes                   |
|                   | `LDR`      | Loads data from memory (Base+offset)        | Yes                   |
|                   | `LEA`      | Loads effective address into register       | Yes                   |
|                   | `ST`       | Stores data to memory (PC-relative)         | No                    |
|                   | `STI`      | Stores data indirectly to memory            | No                    |
|                   | `STR`      | Stores data to memory (Base+offset)         | No                    |
| **Control**       | `BR`       | Conditional branch based on condition codes | No                    |
|                   | `JMP`      | Unconditional jump to address in register   | No                    |
|                   | `JSR/JSRR` | Jump to subroutine                          | No                    |
|                   | `TRAP`     | System call/service call                    | No                    |
|                   | `RTI`      | Return from interrupt                       | No                    |
