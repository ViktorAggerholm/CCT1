...
- [[#11.1 Input/Output Files: Review and Further Study|11.1 Input/Output Files: Review and Further Study]]
	- [[#11.1 Input/Output Files: Review and Further Study#The Keyboard and Screen as Text Streams|The Keyboard and Screen as Text Streams]]
	- [[#11.1 Input/Output Files: Review and Further Study#Newline and EOF|Newline and EOF]]
	- [[#11.1 Input/Output Files: Review and Further Study#Escape Sequences|Escape Sequences]]
	- [[#11.1 Input/Output Files: Review and Further Study#Formatting Output with ``printf``|Formatting Output with ``printf``]]
	- [[#11.1 Input/Output Files: Review and Further Study#File Pointer Variables|File Pointer Variables]]
	- [[#11.1 Input/Output Files: Review and Further Study#Functions That Take File Pointer Arguments|Functions That Take File Pointer Arguments]]
	- [[#11.1 Input/Output Files: Review and Further Study#Closing a File|Closing a File]]
- [[#11.2 Binary Files|11.2 Binary Files]]
- [[#11.3 Searching a Database|11.3 Searching a Database]]
- [[#11.4 Common Programming Errors|11.4 Common Programming Errors]]
...
it has no fixed size. To mark the end of a text file, the computer places a special end-of-file character, which we will denote , after the last character in the file. As you create a text file using an editor program, pressing the or key causes the newline character (represented by C as '\n') to be placed in the file.
...
>
>The disk file consists of a sequence of characters occupying consecutive storage locations on a track of the disk.
>```C
...
However, the input of the special **`<eof>`** character is regarded as a failed operation. The input function responsible returns as its value the negative integer associated with the identifier `EOF`. This special return value gives the calling function an indication that no more data are in the input file. Because of this, the C run-time support system is under no obligation to provide an error message if the program ignores the warning value and continues to attempt to get input from the stream.
...
| `'\b'`          | backspace                                           |
_Figure 1: Common Escape Sequences and their corresponding meanings._
...
Using `'\r'` allows a program to create output where multiple characters appear in the same line position, one after another.

> [!example] Using Escape Sequences in `printf`
> The following call to `printf` displays a heading at the top of a new page, indented to the third tab stop and underlined.
> ```c
> printf("\f\t\t\tFinal Report\r\t\t\t____________\n");
> ```
> **Breakdown of the format string:**
> - `\f`: Issues a form feed, moving to a new page.
> - `\t\t\t`: Issues three tab characters to indent the text.
> - `Final Report`: The text to be displayed.
> - `\r`: Issues a carriage return, moving the cursor back to the beginning of the current line without advancing to the next one.
> - `\t\t\t`: Issues three more tab characters to position the cursor for underlining.
> - `____________`: The underline characters.
> - `\n`: Issues a newline, moving the cursor to the beginning of the next line after the heading is complete.
...
_Figure 1.1: Placeholders for ``printf`` Format Strings_

Additional placeholders can cause the output of integers in octal (base 8) or hexadecimal (base 16), and can display floating-point numbers in scientific notation with either a lowercase or uppercase `e` just before the exponent. 
The scientific notation format produces exactly one nonzero digit to the left of the decimal point. For example, `8.197000e+01` means $8.197000 \times 10^1$. To display a single percent sign, you must place two of them (`%%`) in the format string.
...
> 
> For floating-point values, you can specify both the total field width and the number of decimal digits to the right of the decimal point. This is known as the *precision*. The value will be rounded or padded with trailing zeros to match the prescribed precision. A decimal point occupies a full column, and the precision specification can be used with or without a total field width.
...
The first argument to `fopen` is a string containing the file's name. The second argument is the mode string: `"r"` indicates you intend to read (scan) data from the file, while `"w"` indicates you intend to write to the file as an output destination. The `fopen` function returns a pointer to a structure of type `FILE` that contains the information needed to access the file. This pointer must be saved in a variable of type `FILE*`. The standard streams `stdin`, `stdout`, and `stderr` are also variables of type `FILE*` that are initialized by the system.
...
> ```
> 
> A pointer whose value is `NULL` is called a *null pointer*. This is different from the *null character*, whose value is `'\0'`.

> [!warning] **Risk of Data Loss with "w" Mode**
> 
> Using `fopen` with mode `"w"` to open a file that already exists will usually cause the contents of the existing file to be erased. The only exception is on operating systems that automatically manage file versions by creating a new version when a file is opened for output.

| Example | Meaning of Highlighted Format String Fragment | Output Produced |
| :--- | :--- | :--- |
| `printf("%5d%4d\n", 100, 2);` | Display an integer right-justified in a field of five columns. | ❚❚100❚❚❚2 |
| `printf("%2d with label\n", 5210);` | Display an integer in a field of two columns. Note: Field is too small. | 5210❚with❚label |
...
| `printf("%10.3f\n", 981.48);` | Display a floating-point number right-justified in a field of ten columns, with three digits to the right of the decimal point. | ❚❚❚981.480 |
| `printf("%7.1f\n", 981.48);` | Display a floating-point number right-justified in a field of seven columns, with one digit to the right of the decimal point. | ❚❚981.5 |
| `printf("%12.3e\n", 981.48);` | Display a floating-point number in scientific notation right-justified in a field of 12 columns, with three digits to the right of the decimal point and a lowercase e before the exponent. | ❚❚❚9.815e+02 |
| `printf("%.5E\n", 0.098148);` | Display a floating-point number in scientific notation, with five digits to the right of the decimal point and an uppercase E before the exponent. | 9.81480E-02 |
_Figure 1.2: Designating Field Width, Justification, and Precision in Format Strings_
...
> 
> -   `fscanf` is used for input from a file, much like `scanf` is used for input from the keyboard. For example, to read an integer from a file accessed by the file pointer `infilep`, you would use `fscanf(infilep, "%d", &num)`. Like `scanf`, `fscanf` returns the number of items successfully read, and it returns the `EOF` value when it encounters the end of the file.
> 
...
> 
> A critical detail to remember is the order of arguments. For `fscanf`, `fprintf`, and `getc`, the file pointer is the **first** argument. In contrast, for `putc`, the file pointer is the **second** argument, following the character to be output.
...
_Figure 1.3: Comparison of I/O with Standard Files and I/O with User-Defined File Pointers_
## Closing a File
When a program has finished using a file, it should close the file by calling the library function `fclose` with the appropriate file pointer. For example, `fclose(infilep);` closes the file accessed through `infilep`. This function disposes of the structure that stored file access information and performs other necessary "cleanup" operations.

A program can create an output file and then read from it again. The typical process is to first open the file in `"w"` (write) mode, write data to it using a function like `fprintf`, and then close it using `fclose`. To read the data back, the file is reopened in `"r"` (read) mode, which allows the data to be scanned using a function like `fscanf`.
...
> 
> The program first prompts for the name of the file to back up and tries to open it for reading. It uses a `for` loop that continues to prompt the user until a valid, existing file is successfully opened. It then prompts for a name for the backup file and opens it for writing, using a similar loop to ensure the file can be created.
> 
> The core of the program copies the file one character at a time. It uses a `for` loop with `getc` to read each character from the input file until the end-of-file (`EOF`) marker is encountered. Each character read is then written to the output file using `putc`.
> 
> Finally, the program closes both the input and output files using `fclose` and prints a message to the user confirming that the copy is complete.
> 
> ```c
> /*
...
A *binary file* is created by storing directly in the file the computer’s internal representation of each data component. This bypasses the need for conversion to and from character strings.
...
> 
> The `sizeof` operator is applied to a data type name to find the number of bytes the current implementation uses for storage of that type, making the code more portable.
...
Using binary files offers several advantages:
- **Speed:** Writing the internal binary representation of an integer (e.g., 2 bytes) is much faster than converting it to characters (e.g., "244", which is 4 bytes) and writing each character.
- **Space:** Binary representations typically use less disk space than their character string equivalents.
- **Precision:** When writing a `double` to a text file, precision may be lost during the conversion to a character string. This is avoided with binary files.

> [!warning] **Disadvantages of Binary Files**
> 
> - **Portability:** A binary file created on one type of computer is rarely readable on another, due to different internal data representations.
> - **Human Readability:** A binary file cannot be read or proofread by a human, printed, or examined in a word processor.
> - **Testability:** A program that expects binary input cannot be tested until the program that produces the binary file is complete and working.
...
> 
> The file must be opened with `fopen` using the mode `"rb"` (read binary). `fread` returns an integer indicating how many elements it successfully copied.

> [!important] **Do Not Mix File Types**
> 
> It is critical to use the correct I/O functions for the file type. A binary file created with `fwrite` **must** be read using `fread`. A text file created with `fprintf` **must** be read using a text input function like `fscanf`. Mixing these will lead to errors or corrupted data.

The following table compares the use of text and binary files for common input and output operations. The examples assume the existence of a `planet_t` structure and an array `nums`
...
_Figure 1.5: Data I/O Using Text and Binary Files_
...
In this section, we will write a program that searches a database to find all records that match a proposed set of requirements.
...
>_Structure Chart for Database Inquiry Problem_
>
>**Design**
...
>
>**Program Output**
>*(No program output was provided in the source text)*
>
>**Key Algorithms**
...
>>_Structure Chart for get_params_
>
...
>>_Structure Chart for display_match_
# 11.4 Common Programming Errors
C file processing has several common pitfalls that can lead to errors. It is important to be mindful of the distinctions between file types and the correct functions to use with them.
